...
class CreateUserUseCaseTest {
    ...
    private class MockUserRepository : UserRepository {
        var createResult: Result<User>? = null
        ...
        override suspend fun create(email: String, password: String, fullName: String, role: UserRole): Result<User> =
            createResult ?: Result.Error("Not configured")
    ...
    @Test
    fun `invoke should return success when user is created`() = runTest {
        ...
        mockRepo.createResult = Result.Success(user)

        val useCase = CreateUserUseCase(mockRepo)

        val result = useCase("john.doe@example.com", "Password123", "John Doe", UserRole.CASHIER)

        assertIs<Result.Success<User>>(result)
        assertEquals("user-001", result.data.id)
    }

    @Test
    fun `invoke should return error when email is blank`() = runTest {
        val mockRepo = MockUserRepository()

        val useCase = CreateUserUseCase(mockRepo)

        val result = useCase("", "Password123", "John Doe", UserRole.CASHIER)

        assertIs<Result.Error>(result)
        assertEquals("INVALID_EMAIL", result.code)
    }

    @Test
    fun `invoke should return error when email format is invalid`() = runTest {
        val mockRepo = MockUserRepository()

        val useCase = CreateUserUseCase(mockRepo)

        val result = useCase("invalid-email", "Password123", "John Doe", UserRole.CASHIER)

        assertIs<Result.Error>(result)
        assertEquals("INVALID_EMAIL_FORMAT", result.code)
    }

    @Test
    fun `invoke should return error when password is too short`() = runTest {
        val mockRepo = MockUserRepository()

        val useCase = CreateUserUseCase(mockRepo)

        val result = useCase("john.doe@example.com", "Pass1", "John Doe", UserRole.CASHIER)
    ...
    @Test
    fun `invoke should return error when password lacks complexity`() = runTest {
        val mockRepo = MockUserRepository()
    ...
    fun `invoke should return error when full name is blank`() = runTest {
        ...
        val result = useCase("john.doe@example.com", "Password123", "", UserRole.CASHIER)
