...
class GetUserByIdUseCaseTest {
    ...
    private class MockUserRepository : UserRepository {
        var getByIdResult: Result<User>? = null
        ...
        override suspend fun getById(id: String): Result<User> = getByIdResult ?: Result.Error("Not configured")
    ...
    @Test
    fun `invoke should return user when found`() = runTest {
        ...
        mockRepo.getByIdResult = Result.Success(user)

        val useCase = GetUserByIdUseCase(mockRepo)
    ...
    fun `invoke should return error when user not found`() = runTest {
        val mockRepo = MockUserRepository()
        mockRepo.getByIdResult = Result.Error("User not found", "NOT_FOUND")
