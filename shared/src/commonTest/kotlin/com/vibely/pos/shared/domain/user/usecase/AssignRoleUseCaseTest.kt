...
class AssignRoleUseCaseTest {
    ...
    private class MockUserRepository : UserRepository {
        var assignRoleResult: Result<User>? = null
        ...
        override suspend fun assignRole(id: String, role: UserRole): Result<User> = assignRoleResult ?: Result.Error("Not configured")
    ...
    @Test
    fun `invoke should return success when role is assigned`() = runTest {
        ...
        mockRepo.assignRoleResult = Result.Success(user)

        val useCase = AssignRoleUseCase(mockRepo)

        val result = useCase("user-001", UserRole.MANAGER)

        assertIs<Result.Success<User>>(result)
        assertEquals(UserRole.MANAGER, result.data.role)
    }

    @Test
    fun `invoke should return error when user not found`() = runTest {
        val mockRepo = MockUserRepository()
        mockRepo.assignRoleResult = Result.Error("User not found", "NOT_FOUND")

        val useCase = AssignRoleUseCase(mockRepo)

        val result = useCase("user-999", UserRole.MANAGER)
