...
private val PRODUCT_COLUMNS = Columns.list(
    DatabaseColumns.ID,
    DatabaseColumns.SKU,
...
private suspend fun insertInventoryTransaction(
    ...
    val data = buildJsonObject {
        put(DatabaseColumns.USER_ID, params.userId)
        put(DatabaseColumns.PRODUCT_ID, params.productId)
        put(DatabaseColumns.QUANTITY, params.request.quantity)
        put(DatabaseColumns.TRANSACTION_TYPE, params.request.transactionType)
        put(DatabaseColumns.QUANTITY_AFTER, params.newStock)
        params.request.referenceType?.let { put(DatabaseColumns.REFERENCE_TYPE, it) }
        params.request.referenceId?.let { put(DatabaseColumns.REFERENCE_ID, it) }
        params.request.notes?.let { put(DatabaseColumns.NOTES, it) }
    }
    supabaseClient.from(TableNames.INVENTORY_TRANSACTIONS).insert(data)
}

/**
 * Supabase-backed implementation of [ProductBackendDataSource].
 *
 * Delegates all persistence to Supabase PostgREST using [supabaseClient].
 */
class SupabaseProductDataSource(
    private val supabaseClient: SupabaseClient,
) : BaseService(), ProductBackendDataSource {
    /**
    ...
    override suspend fun getById(id: String): Result<ProductDTO> =
        executeQuery(ERROR_PRODUCT_NOT_FOUND) {
            supabaseClient.from(TableNames.PRODUCTS)
                .select(columns = PRODUCT_COLUMNS) {
                    filter { eq(DatabaseColumns.ID, id) }
                }
                .decodeSingle<ProductDTO>()
        }
    ...
    override suspend fun adjustStock(
        ...
        executeQuery(ERROR_STOCK_ADJUST_FAILED) {
