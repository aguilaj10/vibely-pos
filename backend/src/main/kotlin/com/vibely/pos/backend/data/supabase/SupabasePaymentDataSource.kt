...
class SupabasePaymentDataSource(
    ...
    override suspend fun recordPayment(request: CreatePaymentRequest): Result<PaymentDTO> =
        executeQuery(ERROR_RECORD_PAYMENT) {
            val paymentData = buildJsonObject {
    ...
    override suspend fun getPaymentsBySale(saleId: String): Result<List<PaymentDTO>> =
        executeQuery(ERROR_GET_PAYMENTS) {
            supabaseClient.from(TableNames.PAYMENTS)
                .select {
                    filter { eq(DatabaseColumns.SALE_ID, saleId) }
    ...
    override suspend fun updateSalePaymentStatus(saleId: String): Result<Unit> =
        executeQuery(ERROR_UPDATE_STATUS) {
            ...
            val newPaymentStatus = if (totalPaid >= saleRow.totalAmount) {
